...
首先是最外部的Card组件，一般来说，他需要放在wrapper中，因为在Card中是没有外边距的。
...
FooterButton页面底部按钮，日常开发中，页面底部按钮是使用比较多的场景，因此封装该类型的按钮，并且此按钮为自适应尺寸，灵活改变自身的宽度。具体使用见下方示例。
...
| valueName | string  | setData时间的name参数，在不使用双向绑定时，用来设置字段时间值 |          |          |

##### event

| 事件名  | 说明           | 入参                                                         |
| ------- | -------------- | :----------------------------------------------------------- |
| setData | 时间确认事件   | 对象，例如｛value:"2022-02-11 00:00:00",name:"planStartShippingDate"｝。其中，value：时间字符串，name：props传参valueName的值 |
