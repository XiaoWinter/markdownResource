...
https://code.choerodon.com.cn/happs-tms-op/htms-weixin-demo.git
...
appid是小程序的唯一身份标识，在小程序申请完成后，可以在小程序管理页面，设置菜单中查看到。

![image-20230220162246493](https://typora-huang-cong.oss-cn-shanghai.aliyuncs.com/image-20230220162246493.png)



##### 2.1打开项目文件修改appid以及工程名称

![image-20230220162003436](https://typora-huang-cong.oss-cn-shanghai.aliyuncs.com/image-20230220162003436.png)

##### 2.2使用地图相关功能需要添加腾讯地图key

![image-20230228182608057](https://typora-huang-cong.oss-cn-shanghai.aliyuncs.com/image-20230228182608057.png)

##### 2.3修改环境信息，填入响应的环境信息

![image-20230228182702251](https://typora-huang-cong.oss-cn-shanghai.aliyuncs.com/image-20230228182702251.png)
...
开发者在开发小程序页面时可以使用[vant组件](https://youzan.github.io/vant-weapp/#/home)以及项目自定义组件。

## 自定义组件

为了方面日常的开发,demo小程序工程中封装了以下自定义组件。
...
这5个组件经常一起使用用来构成一个卡片，下面是一个比较完成的示例
	...
	<Card>
		...
		<CardFooter slot="bottom">
			...
			<view slot='btns' class="btns">
				...
				<CardButton bindtap="publishOrder" data-id="{{item.id}}">
					发 布
				</CardButton>
				<CardButton bindtap="cancelOrder" filled data-id="{{item.id}}">
					取 消
				</CardButton>
			</view>
		</CardFooter>
...
![image-20230228102031169](https://typora-huang-cong.oss-cn-shanghai.aliyuncs.com/image-20230228102031169.png)
...
由于在我们的日常使用中，card出现的频率很高，且样式类似，所以Card被封装成的一个组件，它拥有固定的结构，分为上中下三个区域，还具有卡片固定的样式。

Card组件，一般来说，他需要放在wrapper中，因为在Card中是没有外边距的。
...
<view class="wrapper">	
	<Card>
		<view slot='top' style='background:red'>上</view>
		<view slot='mid' style='background:cyan'>中</view>
		<view slot='bottom' style='background:pink'>下</view>
	</Card>
</view>
```

![image-20230228094842610](https://typora-huang-cong.oss-cn-shanghai.aliyuncs.com/image-20230228094842610.png)
...
卡片头部组件，默认用在卡片中，作为card组件的top的slot的默认实现。
	...
	<Card>
		<view slot='top'>
			<CardHeader slot="top">
				...
				<view slot="icon" class="status-text">
					icon
				</view>
				<view slot="status" class="status-text">
					status
...
![image-20230228100349488](https://typora-huang-cong.oss-cn-shanghai.aliyuncs.com/image-20230228100349488.png)
...
卡片底部组件，默认用在卡片中，作为card组件的top的bottom的默认实现。
...
<view class="wrapper">
	<Card>
		...
		<view slot='mid' style='background:cyan'>中</view>
		<CardFooter slot="bottom">
			<view slot="hint" class="text">提示</view>
			<view slot='btns' class="btns">
...
![image-20230228111239645](https://typora-huang-cong.oss-cn-shanghai.aliyuncs.com/image-20230228111239645.png)
...
| ---- | -------------------------------------------- |
| hint | 卡片底部左侧提示信息                         |
| btns | 卡片底部右侧按钮区域，默认使用CardButton组件 |
|      |                                              |

#### 4.CardButton

卡片的标准按钮组件，使用在CardFooter的btns插槽中，具体使用见上CardFooter

##### 源代码

```xml
<view class="{{filled?'filled':'not-filled'}}" hover-class="{{filled?'active-filled':'active-not-filled'}}">
	<slot/>
</view>
```
...
![image-20230228112110221](https://typora-huang-cong.oss-cn-shanghai.aliyuncs.com/image-20230228112110221.png)
...
| 名称   | 类型    | 说明                 | 是否必填 | 默认值 |
| ------ | ------- | -------------------- | -------- | ------ |
| filled | boolean | 是否使用填充颜色按钮 | 否       | false  |



#### 5.CardField

作为卡片中的字段显示使用,用于显示label-value,这两个部分既可以使用props属性，也可以使用插槽，实现方式可以从下方源代码了解
...
<view class="wrapper">
	<Card>
		...
		<view slot='mid'>
			<CardField label="车型1" value="卡车" />
			<CardField>
				<text slot="label">车型1</text>
				<text slot="value">卡车</text>
			</CardField>
			<CardField label="车牌号">
				<text slot="value">沪A111111</text>
			</CardField>
			<CardField value="沪A111111">
				<text slot="label">车牌号</text>
			</CardField>
...
![image-20230228112650000](https://typora-huang-cong.oss-cn-shanghai.aliyuncs.com/image-20230228112650000.png)
...
| 名称  | 类型   | 说明         | 是否必填                 | 默认值 |
| ----- | ------ | ------------ | ------------------------ | ------ |
| label | string | 属性的文本值 | 当没有写label-slot时必填 | 无     |
| value | string | 属性值       | 当没有写value-slot时必填 | 无     |

###### slot

| 名称  | 说明                         |
| ----- | ---------------------------- |
| label | CardField组件的文本值slot    |
| value | CardField组件的value显示slot |
...
假设现在有下面这个值集,值集代码为HTMS.SCAN_TYPE
...
在开发中，我们可以使用这个组件，通过传入值集编码，以及值集value显示直接的meaning文本

```xml
<output lookupCode="HTMS.SCAN_TYPE" value="LOAD" />
```

![image-20230228135152318](https://typora-huang-cong.oss-cn-shanghai.aliyuncs.com/image-20230228135152318.png)
...
| 名称       | 类型   | 说明        | 是否必填 | 默认值 |
| ---------- | ------ | ----------- | -------- | ------ |
| lookupCode | string | 值集编码    | 是       | 无     |
| value      | string | 值集的value | 是       | 无     |



### Steps、Step

自定义步骤条组件，常用于卡片组件,来显示始发地，目的地。Step组件设计有插槽，可以自定义文本内容，以及步骤点内容。
...
```xml
	<Steps>
        <Step>
            <view slot="dot" class="dot bg-qi">起</view>
            <view slot="desc" class="desc">
                上海青浦区汇联路
            </view>
        </Step>
        <Step>
            <view slot="dot" class="dot bg-shi">始</view>
            <view slot="desc" class="desc">上海青浦区汇联路</view>
        </Step>
        <Step removeBottom>
            <view slot="dot" class="dot bg-song">送</view>
            <view slot="desc" class="desc">上海静安区四川北路</view>
        </Step>
    </Steps>
```
...
![image-20230228135922301](https://typora-huang-cong.oss-cn-shanghai.aliyuncs.com/image-20230228135922301.png)
...
### PageFooter,FooterButton

PageFooter页面底部固定组件，用来给固定于页面底部的元素提供一个空间。slot为不具名。

FooterButton一般用作页面底部按钮，日常开发中，页面底部按钮是使用比较多的场景，因此封装该类型的按钮，并且此按钮为自适应尺寸，在PageFooter中放入多个按钮时，FooterButton可以灵活改变自身的宽度。具体使用见下方示例。
...
```xml
<PageFooter>
	<FooterButton bindtap="createOrder">录单</FooterButton>
	<FooterButton filled bindtap="scheduleOrder">调度</FooterButton>
</PageFooter>
```
...
```xml
<PageFooter>
	<FooterButton filled bindtap="scheduleOrder">提交</FooterButton>
</PageFooter>
```
...
![Animation1](https://typora-huang-cong.oss-cn-shanghai.aliyuncs.com/Animation1.gif)



按钮可自适应

![image-20230228142911538](https://typora-huang-cong.oss-cn-shanghai.aliyuncs.com/image-20230228142911538.png)
...
| 名称   | 类型   | 说明               | 是否必输 | 默认值 |
| ------ | ------ | ------------------ | -------- | ------ |
| height | string | 底部占用空间的高度 | 否       | 160rpx |
...
![image-20230228145339576](https://typora-huang-cong.oss-cn-shanghai.aliyuncs.com/image-20230228145339576.png)



### date-picker

封装vant的时间日期组件，用于表单的填写
...
```xml
<date-picker
 value="{planStartShippingDate}"
 label='装车时间'
 required
 type='date'
 bind:setDate="setDateField"
 valueName="planStartShippingDate"
/>

<!-- 表单提交按钮 -->
<PageFooter>
	<FooterButton filled bindtap="submit">提交</FooterButton>
</PageFooter>
```

![Animation1](https://typora-huang-cong.oss-cn-shanghai.aliyuncs.com/Animation.gif)
...
| 名称      | 类型    | 说明                                                         | 是否必输 | 默认值   |
| --------- | ------- | ------------------------------------------------------------ | -------- | -------- |
| label     | string  | 字段名称                                                     | 是       | 无       |
| type      | string  | 同[vant-datetime-picker组件](https://youzan.github.io/vant-weapp/#/datetime-picker)，常用`date`,`time`,`dateTime` | 否       | dateTime |
| value     | string  | 时间字符串，用于初始化时间，以及双向绑定 `model:value='xxx'`需要输入 | 否       | 无       |
| valueName | string  | setData时间的name参数，在不使用双向绑定时，用来设置字段时间值 | 否       | 无       |
| minDate   | string  | 时间最小值，例如`2022-02-11 00:00:00`                        | 否       | 无       |
| maxDate   | string  | 时间最大值，例如`2022-02-11 00:00:00`                        | 否       | 无       |
| disabled  | boolean | 是否禁用                                                     | 否       | false    |
| readonly  | boolean | 是否只读                                                     | 否       | false    |
| clearable | boolean | 是否显示可清除按钮                                           | 否       | true     |
| required  | boolean | 是否显示必输的红点                                           | 否       | false    |
...
| 事件名  | 说明           | 入参                                                         |
| ------- | -------------- | :----------------------------------------------------------- |
| setDate | 时间确认事件   | 对象，例如｛value:"2022-02-11 00:00:00",name:""｝。其中，value：时间字符串，name：props传参valueName的值 |
| confirm | 时间确认       | 数值，时间毫秒值。例如，1677570525543                        |
| cancel  | 时间框关闭时间 | 无                                                           |
...
Page({
    ...
    submit(){
        ...
        const imageList = this.getImgsInfo()
        
    },
	// 通过组件实例调用组件方法
    getImgsInfo() {
        return this.selectComponent("#images").getImgsInfo();
    },
    // 图片消息显示
	handleShowMsg(e) {
        const {
    ...
    showMsg(msg) {
        const msgComp = this.selectComponent("#tip");
        msgComp.showMsg(msg);
    }, 
...
<date-picker
 ...
 required
 type='date'
 model:value='{{planStartShippingDate}}'
/>

<message id="tip"></message>
<imageUpload id="images" sourceKey="{{id}}" sourceType="{{sourceType}}" bindshowMsg="handleShowMsg" />
...
![image-20230228155750680](https://typora-huang-cong.oss-cn-shanghai.aliyuncs.com/image-20230228155750680.png)
...
| 名称       | 类型    | 说明                                | 是否必输 | 默认值   |
| ---------- | ------- | ----------------------------------- | -------- | -------- |
| sourceKey  | string  | 单据id                              | 否       | 无       |
| sourceType | string  | 附件类型，参照TMS系统定义的附件类型 | 是       | 无       |
| title      | string  | 上传附件的名称                      | 否       | 图片上传 |
| total      | numer   | 可上传图片的数量                    | 否       | 9        |
| required   | boolean | 是否显示必输的红点                  | 否       | false    |
...
在提交，调用宏或接口，通常只使用imgIds

```javascript
 {
        imgIds:[1121,1122,1123],
        imgs:[//图片对象信息
        	{
 				id:1121,
                webPath:"hhtps://xxxxx.xxx.xxx/xxax/xx",
                ...
       		}
        ],
};
...
<lov-picker
 ...
 required
 lovCode="HTMS.CUSTOMER_LOV"
 isNeedSearch
 searchPara='name'
 placeholder="请选择"
 valueName="customerGid"
 bind:setLov="setLov"
/>
...
![Animation2](https://typora-huang-cong.oss-cn-shanghai.aliyuncs.com/Animation2.gif)
...
| 名称         | 说明                               | 类型    | 是否必输 | 默认值 |
| ------------ | ---------------------------------- | ------- | -------- | ------ |
| label        | 字段名称                           | string  | 是       | 无     |
| lovCode      | lov值集代码                        | string  | 是       | 无     |
| lovPara      | lov的参数                          | string  | 否       | 无     |
| valueName    | setLov事件的name参数，用来设置字段 | string  | 否       | 无     |
| value        | lov-picker文本显示，可以用双向绑定 | string  | 否       | 无     |
| textField    | lov的文本显示字段                  | string  | 否       | name   |
| valueField   | lov的取值字段                      | string  | 否       | id     |
| placeholder | 输入框提示信息 | string | 否 | 无 |
| disabled     | 是否禁用                           | boolean | 否       | false  |
| readOnly     | 是否只读                           | boolean | 否       | false  |
| required     | 是否显示必填红点                   | boolean | 否       | false  |
| clearable    | 是否显示清除按钮                   | boolean | 否       | true   |
| isNeedSearch | 是否启用lov查询功能                | boolean | 否       | false  |
| errorMessage | 显示的报错信息                     | string  | 否       | 无     |
| searchPara   | 启用lov查询功能时，传递的参数名    | string  | 否       | name   |
...
| 事件名  | 说明                                                         | 回调入参                              |
| ------- | ------------------------------------------------------------ | :------------------------------------ |
| setLov  | lov设置时，触发此事件传递valueField，以及props中传递的valueName，用来做数据的绑定 | ｛value:521,name:'servprovGid'｝      |
| setData | lov设置时，触发此事件传递valueField                          | valueField所指向的lov的值，例如521    |
| clear   | lov清除时，触发事件                                          | 无                                    |
| confirm | lov确认时，触发事件，返回值为textField和valueField所指向的值 | {value: { id:521, text:'整车运输' }｝ |
...
<select-picker
 lookupCode="HTMS.SCAN_TYPE"
 value="LOAD"
 label="扫描方式"
 placeholder="请输入"
 valueName="scanType"
 bind:setLookup="setLookupCode"
/>
...
![Animation3](https://typora-huang-cong.oss-cn-shanghai.aliyuncs.com/Animation3.gif)
...
| ---------- | ------------------------------------- | ------ | -------- | ------ |
| label      | 字段名称                              | string | 是       | 无     |
| lookupCode | lookupCode的值集编码                  | string | 是       | 无     |
| valueName  | setLookup事件的name参数，用来设置字段 | string | 否       | 无     |
| placeholder | 输入框提示信息 | string | 否 | 无 |
| disabled     | 是否禁用                           | boolean | 否       | false  |
| readOnly     | 是否只读                           | boolean | 否       | false  |
| required     | 是否显示必填红点                   | boolean | 否       | false  |
...
| 事件名    | 说明                                                         | 回调入参                         |
| --------- | ------------------------------------------------------------ | :------------------------------- |
| setLookup | lookupCode设置时，触发此事件传递valueField，以及props中传递的valueName，用来做数据的绑定 | ｛value:'LOAD',name:'scanType'｝ |
| clear   | lookupCode清除时，触发事件                                 | 无                                    |
| confirm | lookupCode确认时，触发事件，返回值为textField和valueField所指向的值 | {value: { value:'LOAD', meaning:'装载' }｝ |
...
- 表单页 [页面模版](https://handtms-static.obs.cn-east-3.myhuaweicloud.com:443/component/0/ca4e8ac90f15453abf7e29796972040e@formDemo.rar)
